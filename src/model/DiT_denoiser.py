...
class DiT_Denoiser(nn.Module):
    ...
    def __init__(self,
        ...
        self.cond_pos = PositionalEncoding(self.latent_dim, dropout)
        ...
        if kwargs.get('encoder_layers', 4)==0:
        ...
        # self.source_head = build_mlp(self.latent_dim, 1024, self.latent_dim)
        # self.target_head = build_mlp(self.latent_dim, 1024, self.latent_dim)             
    ...
    def forward(self,
        ...
        tokens = self.dit_encoder(xseq.permute(1, 0, 2), timestep, emb_latent[0])
        # B, T, D
    ...
    def forward_with_attention(self,
        ...
        # motion_embeds: T, B, D
        # len: B
        # condition_mask: B, T_text+T_source
        ...
        if motion_embeds is None:
            ...
            sep_token_mask = torch.ones((bs, self.sep_token.shape[0]),
                                        dtype=bool,
                                        device=noised_motion.device) # B, 1
            aug_mask = torch.cat((
                            condition_mask[:, text_emb_latent.shape[0]:],
                            sep_token_mask,
                            motion_in_mask,
        ...
        xseq = self.query_pos(xseq) # T, 1, D
        tokens, masks = self.dit_encoder.forward_with_attention(xseq.permute(1, 0, 2), timestep, emb_latent[0])
        # B, T, D
        tokens = tokens.permute(1, 0, 2)
        # if self.diffusion_only:
        if motion_embeds is not None:
    ...
    #     xseq = self.query_pos(xseq)
    #     tokens, target_repr = self.dit_encoder.forward_with_repr(xseq.permute(1, 0, 2), timestep, emb_latent[0])
    #     # B, T, D
    #     tokens = tokens.permute(1, 0, 2)
    #     # if self.diffusion_only:
    #     if motion_embeds is not None:
    ...
    #     target_repr = self.target_head(target_repr[:, :T_Text + T_max]) # B, T_target, D
    #     source_repr = self.source_head(motion_embeds_proj) # B, T_source, D
    #     return denoised_motion, source_repr, target_repr
