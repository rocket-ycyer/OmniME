...
class MotionFixDataset(Dataset):
    ...
    def random_index_excluding(self, idx: int, N: int) -> int:
        assert 0 <= idx < N, "idx must be within [0, N)"
        if N <= 1:
            raise ValueError("N must be greater than 1 to exclude idx")

        rand_idx = random.randint(0, N - 2)
        if rand_idx >= idx:
            rand_idx += 1
        return rand_idx
    ...
    def __getitem__(self, idx):
        datum = self.data[idx]
                # -----------------随机抽样neg text-----------------
        neg_idx = self.random_index_excluding(idx, len(self.data)) #@##########
        neg_text = self.data[neg_idx]['text'] #@##########
