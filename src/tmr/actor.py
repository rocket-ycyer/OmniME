...
class ACTORStyleEncoder(nn.Module):
    ...
    def forward_repr(self, x_dict: Dict) -> Tensor:
        # output: B, T, D
        x = x_dict["x"]
        mask = x_dict["mask"]
        ...
        # add positional encoding
        xseq = self.sequence_pos_encoding(xseq)
        for layer in self.seqTransEncoder.layers[:self.num_layers-1]: # 6 layers in total
            xseq = layer(xseq, src_key_padding_mask=~aug_mask)
        return xseq[:, self.nbtokens:]
