import os
import sys
sys.path.append('/data1/home/Modelzhenwu1/SimMotionEdit')
import logging
import hydra
import joblib
...
#----------------------------------------------------------------
    my_id = 'paired_data_seed0_15_batch17_id150'
    my_text = 'bend down'
    ...
    for idx in range(len(test_dataset)):
        ...
        if sample['id'] == my_id:
            target_index = idx
            modified_sample = dict(sample)  # 复制样本
            modified_sample['text'] = my_text  # 修改 text
            break

    if target_index is None:
            raise ValueError("Sample with id '003696' not found")
    ...
    test_dataset = SingleSampleDataset(modified_sample)
        ...
        for guid_text, guid_motion in guidances_mix:
            ...
            for batch in tqdm(ds_iterator):
                ...
                for i in range(gen_mo.shape[0]):
                    ...
                    np.save("/data2/home/Modelzhenwu1/edit.npy",
                            dict_to_save)
